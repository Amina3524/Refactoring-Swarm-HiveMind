...
def log_experiment(agent_name: str, model_used: str, action: ActionType, details: dict, status: str):
    ...
    if isinstance(action, ActionType):
        ...
        raise ValueError(f"[ERROR] Action invalide : '{action}'. Utilisez la classe ActionType (ex: ActionType.FIX).")
    ...
    if action_str in [ActionType.ANALYSIS, ActionType.GENERATION, ActionType.DEBUG, ActionType.FIX]:
        ...
        if missing_keys:
            raise ValueError(
                f"[ERROR] Logging (Agent: {agent_name}) : "
    ...
    if os.path.exists(LOG_FILE):
        try:
            ...
            print(f"[WARNING] Le fichier de logs {LOG_FILE} était corrompu. Une nouvelle liste a été créée.")
            data = []
