...
4. Maintain the original functionality - DO NOT change what the code does
...
IMPORTANT: Your response will be written directly to a .py file, so it must be pure Python code only."""
    ...
    def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not audit_report:
            ...
            audit_report = {"summary": {"refactoring_priority": ["Basic cleanup"]}}
        ...
        if not syntax_check["valid"]:
            self._log(f"⚠️  Fixed code has syntax error: {syntax_check['error']}")
        ...
        log_experiment(
            ...
            status="SUCCESS" if syntax_check["valid"] else "PARTIAL"
        )
    ...
    def _attempt_syntax_fix(self, code: str, syntax_check: Dict) -> str:
        ...
        self._log("⚠️  Cannot auto-fix syntax error, returning as-is")
        return code
