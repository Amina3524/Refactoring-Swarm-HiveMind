...
def validate_strict_format() -> Tuple[bool, List[str], Dict[str, Any]]:
    ...
    print("🔍 VALIDATION STRICTE DES LOGS - CRITÈRES TP")
    print("=" * 70)
    ...
    valid_actions = ['CODE_ANALYSIS', 'CODE_GEN', 'DEBUG', 'FIX']  # VOTRE ActionType
    valid_statuses = ['SUCCESS', 'FAILURE', 'ERROR']
    
    # Agents minimum requis par le TP
    required_agents = {'Auditor', 'Fixer', 'Judge', 'Toolsmith'}
    agents_found = set()
    ...
    for i, entry in enumerate(logs):
        ...
        if 'status' in entry:
            ...
            if status not in valid_statuses:
                entry_errors.append(f"Status invalide: '{status}'")
            else:
        ...
        if 'details' in entry:
            ...
            if not isinstance(details, dict):
                ...
                if 'input_prompt' not in details:
                    entry_errors.append("'input_prompt' manquant dans details (OBLIGATOIRE)")
                if 'output_response' not in details:
                    entry_errors.append("'output_response' manquant dans details (OBLIGATOIRE)")
                
                # Vérifier la qualité des prompts
                if 'input_prompt' in details:
                    prompt = str(details['input_prompt']).strip()
                    if len(prompt) < 10:
                        entry_warnings.append(f"'input_prompt' très court ({len(prompt)} chars)")
                    all_prompts.append(prompt[:200])  # Garder un échantillon
                
                if 'output_response' in details:
                    response = str(details['output_response']).strip()
                    if len(response) < 5:
                        entry_warnings.append(f"'output_response' très court ({len(response)} chars)")
                    all_responses.append(response[:200])
                
                # === CRITÈRE 8: SÉCURITÉ SANDBOX ===
                # Vérifier qu'on n'écrit pas hors sandbox
                for key, value in details.items():
                    if isinstance(value, str) and '..' in value:
                        if 'sandbox' not in value and 'test' not in value.lower():
        ...
        # Collecter les erreurs de cette entrée
        if entry_errors:
    ...
    missing_agents = required_agents - {agent.split('_')[0] for agent in agents_found if '_' in agent}
    if missing_agents:
    ...
    # Vérifier la diversité des prompts
    if len(set(all_prompts)) < len(all_prompts) * 0.3:  # Moins de 30% uniques
    ...
    total_success = statistics['by_status'].get('SUCCESS', 0)
    success_rate = (total_success / statistics['total_entries']) * 100 if statistics['total_entries'] > 0 else 0
    print(f"    • SUCCESS: {total_success} ({success_rate:.1f}%)")
    
    # ===== CRITÈRE FINAL: PRÊT POUR ÉVALUATION =====
    is_ready_for_evaluation = len(errors) == 0
    ...
    if is_ready_for_evaluation:
        ...
        print("   Corriger les erreurs avant la soumission.")
...
def save_validation_report(errors: List[str], statistics: Dict[str, Any]) -> str:
    ...
    if statistics.get("total_entries", 0) < 10:
        report["recommendations"].append("Ajouter plus d'entrées de log (minimum 10 recommandé)")
...
def main():
    ...
    # Créer un résumé pour la console
    print(f"\n📋 RÉSUMÉ EXÉCUTIF:")
    ...
    print(f"   Taux de succès: {statistics.get('by_status', {}).get('SUCCESS', 0)}/{statistics.get('total_entries', 0)}")
